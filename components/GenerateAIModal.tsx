
import React from 'react';
import { useTextValidation } from '../src/hooks/useValidation';

interface GenerateAIModalProps {
  isOpen: boolean;
  onClose: () => void;
  prompt: string;
  setPrompt: (prompt: string) => void;
  onGenerate: () => void;
  isLoading: boolean;
}

const GenerateAIModal: React.FC<GenerateAIModalProps> = ({
  isOpen,
  onClose,
  prompt,
  setPrompt,
  onGenerate,
  isLoading,
}) => {
  // Input validation for the prompt
  const {
    value: validatedPrompt,
    setValue: setValidatedPrompt,
    validation,
    hasErrors
  } = useTextValidation(prompt, {
    maxLength: 2000,
    minLength: 10,
    allowHtml: false,
    allowSpecialChars: true,
    validateOnChange: true,
    debounceMs: 300
  });

  // Sync with parent component
  React.useEffect(() => {
    if (validatedPrompt !== prompt) {
      setPrompt(validatedPrompt);
    }
  }, [validatedPrompt, prompt, setPrompt]);

  if (!isOpen) return null;

  return (
    <div
      className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center"
      aria-modal="true"
      role="dialog"
      onClick={onClose}
    >
      <div
        className="bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl transform transition-all p-6 border border-gray-700"
        onClick={(e) => e.stopPropagation()} // Prevent closing when clicking inside
      >
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-white">Generate Diagram with AI</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-white">&times;</button>
        </div>
        <p className="text-gray-400 mb-4 text-sm">
          Describe the diagram you want to create. Mention specific technologies (e.g., "AWS S3", "Cisco Router") to include their icons if available in the selected icon set.
        </p>
        <div className="space-y-2">
          <textarea
            value={validatedPrompt}
            onChange={(e) => setValidatedPrompt(e.target.value)}
            className={`w-full h-40 p-3 bg-gray-900 text-gray-200 font-mono resize-y rounded-md focus:outline-none focus:ring-2 border ${
              hasErrors
                ? 'border-red-500 focus:ring-red-500'
                : 'border-gray-600 focus:ring-cyan-500'
            }`}
            placeholder="e.g., A simple flowchart with a user login process."
            disabled={isLoading}
            maxLength={2000}
          />
          {hasErrors && (
            <div className="text-red-400 text-sm space-y-1">
              {validation.errors.map((error, index) => (
                <p key={index}>• {error}</p>
              ))}
            </div>
          )}
          <div className="flex justify-between text-xs text-gray-500">
            <span>{validatedPrompt.length}/2000 characters</span>
            {validatedPrompt.length < 10 && (
              <span className="text-red-400">Minimum 10 characters required</span>
            )}
          </div>
        </div>
        <div className="mt-6 flex justify-end gap-4">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm font-medium text-gray-300 bg-gray-600 rounded-md hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-gray-500 transition-colors"
            disabled={isLoading}
          >
            Cancel
          </button>
          <button
            onClick={onGenerate}
            className="px-4 py-2 text-sm font-medium text-white bg-teal-600 rounded-md hover:bg-teal-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-teal-500 transition-colors disabled:bg-teal-400 disabled:cursor-not-allowed flex items-center"
            disabled={isLoading || !validatedPrompt.trim() || hasErrors}
          >
            {isLoading ? (
              <>
                <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                Generating...
              </>
            ) : (
              'Generate'
            )}
          </button>
        </div>
      </div>
    </div>
  );
};

export default GenerateAIModal;
